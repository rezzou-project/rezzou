...
function createContextProxy(callId: string, repo: Repo, provider: Provider): RepoContext {
  ...
  return {
    repo,
    provider,
    readFile: (filePath) => proxyCall("readFile", [filePath]) as Promise<string | null>,
    listFiles: (glob) => proxyCall("listFiles", [glob]) as Promise<string[]>,
    exists: (filePath) => proxyCall("exists", [filePath]) as Promise<boolean>
  };
}
...
async function handleMessage(msg: IncomingMessage): Promise<void> {
  ...
  if (msg.type === "call-apply") {
    const operation = plugin.operations.find((op: Operation) => op.id === msg.operationId);
    if (!operation) {
  ...
  if (msg.type === "call-branchName") {
    await callOperationStringFn({
      callId: msg.callId, operationId: msg.operationId, fnName: "branchName", inputs: msg.inputs
    });
